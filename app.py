...
# Initialize session state for accumulated alerts
if 'accumulated_alerts' not in st.session_state:
    st.session_state['accumulated_alerts'] = {}

# Button to clear accumulated alerts (optional, for resetting)
...
if st.button("Generate Alerts"):
    if threshold_df is not None and new_file is not None:
        ...
        if not final_alerts.empty:
            ...
            st.session_state['accumulated_alerts'][selected_province] = final_alerts
            st.success(f"Alerts for {selected_province} updated in accumulated map data.")
...
if st.session_state['accumulated_alerts']:
    ...
    if not combined_alerts.empty:
        ...
        if selected_diseases:
            map_data = combined_alerts[combined_alerts['Disease'].isin(selected_diseases)]
        else:
        ...
        if not map_data.empty:
            ...
            color_map = {'Alert': 'orange', 'High Alert': 'red'}
            
            for idx, row in map_data.iterrows():
                popup_text = f"""
                Facility: {row['Facility_ID']}<br>
                Disease: {row['Disease']}<br>
                Cases: {row['Cases']}<br>
                Deviation: {row['Deviation']}<br>
