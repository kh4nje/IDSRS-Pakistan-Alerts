...
if st.button("Generate Alerts"):
    if threshold_df is not None and new_file is not None:
        ...
        # Define possible columns to keep
        disease_list = [
            "Acute Diarrhea (Non-Cholera) (New Cases)",
            "Acute Flaccid Paralysis (New Cases)",
            "Acute Watery Diarrhea (Suspected Cholera) (New Cases)",
            "Anthrax (New Cases)",
            "Bloody Diarrhea (New Cases)",
            "Botulism (New Cases)",
            "Brucellosis (New cases)",
            "COVID-19 (New Cases)",
            "Chickenpox/ Varicella (New cases)",
            "Chikungunya (New Cases)",
            "Crimean Congo Hemorrhagic Fever (New Cases)",
            "Cutaneous Leishmaniasis (New Cases)",
            "Dengue Fever (New Cases)",
            "Diphtheria (Probable) (New Cases)",
            "Dog Bite (New Cases)",
            "Encephalitis (New Cases)",
            "Gonorrhea (New Cases)",
            "HIV/AIDS (New Cases)",
            "Influenza-Like Illness (New Cases)",
            "Leprosy (New Cases)",
            "Malaria (New Cases)",
            "Measles (New Cases)",
            "Meningitis (New Cases)",
            "Mumps (New Cases)",
            "Neonatal Tetanus (New Cases)",
            "Nosocomial Infections (New Cases)",
            "Other-1 (New Cases)",
            "Other-2 (New Cases)",
            "Pertussis (New cases)",
            "Pneumonia/ALRI (Acute Lower Respiratory Infections) under 5 years (New Cases)",
            "Rubella (Congenital Rubella Syndrome (CRS)) (New Cases)",
            "Salmonellosis (New Cases)",
            "Scabies (New Cases)",
            "Severe Acute Respiratory Infection (New Cases)",
            "Syphilis (New Cases)",
            "Tuberculosis (New Cases)",
            "Typhoid Fever (New Cases)",
            "Viral Hepatitis (B, C & D) (New Cases)",
            "Visceral Leishmaniasis (New Cases)"
        ]
        possible_columns = [
            'periodname', 'orgunitlevel2', 'orgunitlevel3', 'orgunitlevel4', 'orgunitlevel5', 'organisationunitname'
        ] + disease_list
        
        # Filter to only desired columns that exist
        existing_columns = [col for col in possible_columns if col in new_df.columns]
        new_df = new_df[existing_columns]
        
        # Define org columns based on what exists
        org_level_cols = [col for col in ['orgunitlevel2', 'orgunitlevel3', 'orgunitlevel4', 'orgunitlevel5'] if col in new_df.columns]
        org_cols = org_level_cols + (['organisationunitname'] if 'organisationunitname' in new_df.columns else [])
        
        # Fill NaN and create Facility_ID if org_cols exist
        if org_cols:
            for col in org_cols:
        ...
        # Disease columns (only those in disease_list that exist)
        disease_cols = [col for col in new_df.columns if col in disease_list]
        ...
        # Melt, including separate org_cols in id_vars
        id_vars = ['Facility_ID', 'Season'] + org_cols
        long_new = pd.melt(new_df, id_vars=id_vars, value_vars=disease_cols, var_name='Disease', value_name='Cases')
        ...
        # Select columns for final_alerts, using separate org_cols instead of Facility_ID
        final_columns = org_cols + ['Disease', 'Season', 'Cases', 'Mean', 'SD', 'Threshold_95', 'Threshold_99', 'Alert_Level', 'Deviation']
        alerts = alerts[final_columns]
